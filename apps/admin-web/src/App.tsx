...
import type { CreateStaffUserInput, IdentitySession, RoleScope } from "@spaceman/app-types";
...
export function App({ identityService, identityAdminService }: AppProps) {
  ...
  const access = evaluateIdentityAccess(session, ["admin", "super_admin"], false);
  ...
  return (
    <main className="shell">
      ...
      <p className="lead">
        Admin access is invitation-only, backed by canonical profiles and server-issued claims.
      </p>
      ...
      {!loading && session && (access.reason === "inactive" || access.reason === "wrong_role") ? (
        <section className="account-panel">
          ...
          <p>Role: {role ?? "missing"} · Status: {session.profile?.status ?? "missing"}.</p>
      ...
      {access.granted ? (
        <>
          <div className="account-heading">
            <p>Signed in as {session?.email} ({role}).</p>
          ...
          <section aria-label="Admin scope" className="grid">
            {operationalAreas.map((area) => (
              <article className="card" key={area}>
                <h2>{area}</h2>
                <p>Role-, scope-, and transition-guarded workflows use trusted Functions.</p>
              </article>
            ))}
          </section>
          <section className="account-panel">
            ...
            <div className="forms">
              {role === "super_admin" ? (
                <form onSubmit={(event) => void inviteStaff(event)}>
                  ...
                  <label>Role<select name="role"><option value="merchant">Merchant</option><option value="driver">Driver</option><option value="admin">Admin</option></select></label>
                  <label>Store IDs<input name="storeIds" placeholder="store-1, store-2" /></label>
                  <label>Delivery zone IDs<input name="deliveryZoneIds" /></label>
                  <label>Region IDs<input name="regionIds" /></label>
                  <button disabled={busy} type="submit">Invite staff account</button>
                </form>
              ) : null}
              <form onSubmit={(event) => void updateStatus(event)}>
                ...
                <button disabled={busy} type="submit">Update status</button>
              </form>
              <form onSubmit={(event) => void updateScope(event)}>
                <h3>Replace account scope</h3>
                <label>User ID<input name="userId" required /></label>
                <label>Store IDs<input name="storeIds" /></label>
                <label>Delivery zone IDs<input name="deliveryZoneIds" /></label>
                <label>Region IDs<input name="regionIds" /></label>
                <button disabled={busy} type="submit">Update scope</button>
              </form>
            </div>
          </section>
        </>
